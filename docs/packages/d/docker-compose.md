# [docker-compose](https://pypi.org/project/docker-compose)

## Dependencies
- [backports.ssl-match-hostname (<4,>=3.5); python_version<'3.5'](packages/b/backports.ssl-match-hostname.md)
- [cached-property (<2,>=1.2.0)](packages/c/cached-property.md)
- [colorama (<1,>=0.4); sys_platform=='win32'](packages/c/colorama.md)
- [distro (<2,>=1.5.0)](packages/d/distro.md)
- [docker[ssh] (<5,>=4.3.1)](packages/d/docker.md)
- [dockerpty (<1,>=0.4.1)](packages/d/dockerpty.md)
- [docopt (<1,>=0.6.1)](packages/d/docopt.md)
- [jsonschema (<4,>=2.5.1)](packages/j/jsonschema.md)
- [python-dotenv (<1,>=0.13.0)](packages/p/python-dotenv.md)
- [pyyaml (<6,>=3.10)](packages/p/pyyaml.md)
- [requests (<3,>=2.20.0)](packages/r/requests.md)
- [texttable (<2,>=0.9.0)](packages/t/texttable.md)
- [websocket-client (<1,>=0.32.0)](packages/w/websocket-client.md)


## Extras

### socks
- [pysocks (!=1.5.7,<2,>=1.5.6); extra=='socks'](packages/p/pysocks.md)


## Publishers
- [ccrone](https://pypi.org/user/ccrone)
- [dev-tooling-team](https://pypi.org/user/dev-tooling-team)
- [docker](https://pypi.org/user/docker)
- [rumpl](https://pypi.org/user/rumpl)
- [slubecki](https://pypi.org/user/slubecki)
- [ulyssessouza](https://pypi.org/user/ulyssessouza)

