# [dagster](https://pypi.org/project/dagster)

## Dependencies
- [alembic (>=1.2.1)](packages/a/alembic.md)
- [backports.tempfile; python_version<'3'](packages/b/backports.tempfile.md)
- [click (>=5.0)](packages/c/click.md)
- [coloredlogs (<=14.0,>=6.1)](packages/c/coloredlogs.md)
- [contextlib2 (>=0.5.4)](packages/c/contextlib2.md)
- [croniter (>=0.3.34)](packages/c/croniter.md)
- [docstring-parser (==0.7.1); python_version>='3.6'](packages/d/docstring-parser.md)
- [enum34; python_version<'3.4'](packages/e/enum34.md)
- [funcsigs](packages/f/funcsigs.md)
- [functools32; python_version<'3'](packages/f/functools32.md)
- [future](packages/f/future.md)
- [futures; python_version<'3'](packages/f/futures.md)
- [grpcio (>=1.32.0)](packages/g/grpcio.md)
- [grpcio-health-checking (>=1.32.0)](packages/g/grpcio-health-checking.md)
- [pathlib2 (>=2.3.4); python_version<'3'](packages/p/pathlib2.md)
- [pendulum (==1.4.4)](packages/p/pendulum.md)
- [protobuf (>=3.13.0)](packages/p/protobuf.md)
- [psutil (>=1.0); platform_system=='windows'](packages/p/psutil.md)
- [pyrsistent (<=0.16.0,>=0.14.8); python_version<'3'](packages/p/pyrsistent.md)
- [pyrsistent (>=0.14.8); python_version>='3'](packages/p/pyrsistent.md)
- [python-dateutil](packages/p/python-dateutil.md)
- [pytz](packages/p/pytz.md)
- [pywin32 (!=226); platform_system=='windows'](packages/p/pywin32.md)
- [pyyaml](packages/p/pyyaml.md)
- [requests](packages/r/requests.md)
- [rx (<=1.6.1)](packages/r/rx.md)
- [six](packages/s/six.md)
- [sqlalchemy (>=1.0)](packages/s/sqlalchemy.md)
- [tabulate](packages/t/tabulate.md)
- [toposort (>=1.0)](packages/t/toposort.md)
- [tqdm](packages/t/tqdm.md)
- [typing; python_version<'3'](packages/t/typing.md)
- [watchdog (>=0.8.3)](packages/w/watchdog.md)


## Extras

### docker
- [docker; extra=='docker'](packages/d/docker.md)


## Publishers
- [elementl](https://pypi.org/user/elementl)
- [mgasner](https://pypi.org/user/mgasner)

