# [tensorflow-data-validation](https://pypi.org/project/tensorflow-data-validation)

## Dependencies
- [absl-py (<0.11,>=0.9)](packages/a/absl-py.md)
- [apache-beam[gcp] (<3,>=2.25)](packages/a/apache-beam.md)
- [joblib (<0.15,>=0.12)](packages/j/joblib.md)
- [numpy (<2,>=1.16)](packages/n/numpy.md)
- [pandas (<2,>=1.0)](packages/p/pandas.md)
- [protobuf (<4,>=3.9.2)](packages/p/protobuf.md)
- [pyarrow (<0.18,>=0.17)](packages/p/pyarrow.md)
- [six (<2,>=1.12)](packages/s/six.md)
- [tensorflow (!=2.0.*,!=2.1.*,!=2.2.*,<3,>=1.15.2)](packages/t/tensorflow.md)
- [tensorflow-metadata (<0.26,>=0.25)](packages/t/tensorflow-metadata.md)
- [tensorflow-transform (<0.26,>=0.25)](packages/t/tensorflow-transform.md)
- [tfx-bsl (<0.26,>=0.25)](packages/t/tfx-bsl.md)


## Extras

### mutual-information
- [scikit-learn (<0.24,>=0.18); extra=='mutual-information'](packages/s/scikit-learn.md)

### visualization
- [ipython (<8,>=7); extra=='visualization'](packages/i/ipython.md)


## Publishers
- [google_opensource](https://pypi.org/user/google_opensource)
- [tfx_oss](https://pypi.org/user/tfx_oss)

