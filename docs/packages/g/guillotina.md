# [guillotina](https://pypi.org/project/guillotina)

## Dependencies
- [argon2-cffi](packages/a/argon2-cffi.md)
- [asyncpg](packages/a/asyncpg.md)
- [backoff](packages/b/backoff.md)
- [cffi](packages/c/cffi.md)
- [jsonschema](packages/j/jsonschema.md)
- [jwcrypto](packages/j/jwcrypto.md)
- [lru-dict](packages/l/lru-dict.md)
- [multidict](packages/m/multidict.md)
- [mypy-extensions](packages/m/mypy-extensions.md)
- [orjson (<4,>=3)](packages/o/orjson.md)
- [pycryptodome](packages/p/pycryptodome.md)
- [pyjwt (<2.0.0)](packages/p/pyjwt.md)
- [python-dateutil](packages/p/python-dateutil.md)
- [pyyaml (>=5.1)](packages/p/pyyaml.md)
- [setuptools](packages/s/setuptools.md)
- [typing-extensions](packages/t/typing-extensions.md)
- [uvicorn](packages/u/uvicorn.md)
- [websockets](packages/w/websockets.md)
- [zope.interface](packages/z/zope.interface.md)


## Extras

### mailer
- [aiosmtplib (>=1.0.6); extra=='mailer'](packages/a/aiosmtplib.md)
- [html2text (>=2018.1.9); extra=='mailer'](packages/h/html2text.md)

### recaptcha
- [aiohttp (<4); extra=='recaptcha'](packages/a/aiohttp.md)

### redis
- [aioredis (>=1.2.0); (python_version<'3.8') and extra=='redis'](packages/a/aioredis.md)
- [aioredis (>=1.3.0); (python_version>='3.8') and extra=='redis'](packages/a/aioredis.md)

### testdata
- [aiohttp (<3.6.0,>=3.0.0); (python_version<'3.8') and extra=='testdata'](packages/a/aiohttp.md)
- [aiohttp (<4.0.0,>=3.6.0); (python_version>='3.8') and extra=='testdata'](packages/a/aiohttp.md)

### validation
- [pytz (==2020.1); extra=='validation'](packages/p/pytz.md)


## Publishers
- [ramon.nb](https://pypi.org/user/ramon.nb)
- [vangheem](https://pypi.org/user/vangheem)

