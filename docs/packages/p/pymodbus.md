# [pymodbus](https://pypi.org/project/pymodbus)

## Dependencies
- [pyserial (>=3.4)](packages/p/pyserial.md)
- [six (>=1.15.0)](packages/s/six.md)


## Extras

### documents
- [humanfriendly; extra=='documents'](packages/h/humanfriendly.md)
- [sphinx (>=1.1.3); extra=='documents'](packages/s/sphinx.md)
- [sphinx-rtd-theme; extra=='documents'](packages/s/sphinx-rtd-theme.md)

### quality
- [coverage (>=3.5.3); extra=='quality'](packages/c/coverage.md)
- [mock (>=1.0.0); extra=='quality'](packages/m/mock.md)
- [nose (>=1.2.1); extra=='quality'](packages/n/nose.md)
- [pep8 (>=1.3.3); extra=='quality'](packages/p/pep8.md)

### repl
- [click (>=7.0); (python_version<='2.7') and extra=='repl'](packages/c/click.md)
- [click (>=7.0); (python_version>='3.6') and extra=='repl'](packages/c/click.md)
- [prompt-toolkit (==2.0.4); (python_version<='2.7') and extra=='repl'](packages/p/prompt-toolkit.md)
- [prompt-toolkit (>=3.0.8); (python_version>='3.6') and extra=='repl'](packages/p/prompt-toolkit.md)
- [pygments (>=2.2.0); (python_version<='2.7') and extra=='repl'](packages/p/pygments.md)
- [pygments (>=2.2.0); (python_version>='3.6') and extra=='repl'](packages/p/pygments.md)

### tornado
- [tornado (==4.5.3); extra=='tornado'](packages/t/tornado.md)

### twisted
- [pyasn1 (>=0.1.4); extra=='twisted'](packages/p/pyasn1.md)
- [twisted (>=20.3.0); extra=='twisted'](packages/t/twisted.md)


## Publishers
- [bashwork](https://pypi.org/user/bashwork)
- [dhoomakethhu](https://pypi.org/user/dhoomakethhu)

