# [aws-sam-cli](https://pypi.org/project/aws-sam-cli)

## Dependencies
- [aws-lambda-builders (==1.1.0)](packages/a/aws-lambda-builders.md)
- [aws-sam-translator (==1.32.0)](packages/a/aws-sam-translator.md)
- [boto3 (~=1.14.23)](packages/b/boto3.md)
- [chevron (~=0.12)](packages/c/chevron.md)
- [click (~=7.1)](packages/c/click.md)
- [cookiecutter (~=1.7.2)](packages/c/cookiecutter.md)
- [dateparser (~=0.7)](packages/d/dateparser.md)
- [docker (~=4.2.0)](packages/d/docker.md)
- [flask (~=1.1.2)](packages/f/flask.md)
- [jmespath (~=0.10.0)](packages/j/jmespath.md)
- [python-dateutil (<2.8.1,~=2.6)](packages/p/python-dateutil.md)
- [pyyaml (~=5.3)](packages/p/pyyaml.md)
- [requests (==2.23.0)](packages/r/requests.md)
- [serverlessrepo (==0.1.10)](packages/s/serverlessrepo.md)
- [tomlkit (==0.7.0)](packages/t/tomlkit.md)
- [watchdog (==0.10.3)](packages/w/watchdog.md)



## Publishers
- [aws-sam-developers](https://pypi.org/user/aws-sam-developers)

