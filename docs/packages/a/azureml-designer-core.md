# [azureml-designer-core](https://pypi.org/project/azureml-designer-core)

## Dependencies
- [azureml-core (==1.18.0.*)](packages/a/azureml-core.md)
- [distro (==1.4.0)](packages/d/distro.md)
- [jsonschema (==3.0.1)](packages/j/jsonschema.md)
- [more-itertools (==6.0.0)](packages/m/more-itertools.md)
- [numpy (==1.18.1)](packages/n/numpy.md)
- [pandas (==1.0.4)](packages/p/pandas.md)
- [pyarrow (==0.16.0)](packages/p/pyarrow.md)
- [pycryptodomex (==3.7.3)](packages/p/pycryptodomex.md)
- [python-dateutil (==2.8.1)](packages/p/python-dateutil.md)
- [ruamel.yaml (==0.16.10)](packages/r/ruamel.yaml.md)


## Extras

### image
- [pillow (==7.1.0); extra=='image'](packages/p/pillow.md)

### model
- [cloudpickle (==1.2.2); extra=='model'](packages/c/cloudpickle.md)


## Publishers
- [amlpypi](https://pypi.org/user/amlpypi)

