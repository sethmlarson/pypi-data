# [allennlp](https://pypi.org/project/allennlp)

## Dependencies
- [boto3 (<2.0,>=1.14)](packages/b/boto3.md)
- [dataclasses; python_version<'3.7'](packages/d/dataclasses.md)
- [filelock (<3.1,>=3.0)](packages/f/filelock.md)
- [h5py](packages/h/h5py.md)
- [jsonnet (>=0.10.0); sys_platform !='win32'](packages/j/jsonnet.md)
- [jsonpickle](packages/j/jsonpickle.md)
- [nltk](packages/n/nltk.md)
- [numpy](packages/n/numpy.md)
- [overrides (==3.1.0)](packages/o/overrides.md)
- [pytest](packages/p/pytest.md)
- [requests (>=2.18)](packages/r/requests.md)
- [scikit-learn](packages/s/scikit-learn.md)
- [scipy](packages/s/scipy.md)
- [sentencepiece](packages/s/sentencepiece.md)
- [spacy (<2.4,>=2.1.0)](packages/s/spacy.md)
- [tensorboardx (>=1.2)](packages/t/tensorboardx.md)
- [torch (<1.8.0,>=1.6.0)](packages/t/torch.md)
- [tqdm (>=4.19)](packages/t/tqdm.md)
- [transformers (<4.1,>=4.0)](packages/t/transformers.md)



## Publishers
- [allennlp](https://pypi.org/user/allennlp)
- [dirkgr](https://pypi.org/user/dirkgr)
- [epwalsh](https://pypi.org/user/epwalsh)

