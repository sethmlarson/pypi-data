# [satella](https://pypi.org/project/satella)

## Dependencies
- [psutil](packages/p/psutil.md)


## Extras

### cassandra
- [cassandra-driver; extra=='cassandra'](packages/c/cassandra-driver.md)

### fasterjson
- [ujson; extra=='fasterjson'](packages/u/ujson.md)

### httpjsonsource
- [requests; extra=='httpjsonsource'](packages/r/requests.md)

### opentracing
- [opentracing; extra=='opentracing'](packages/o/opentracing.md)

### tomlsource
- [toml; extra=='tomlsource'](packages/t/toml.md)

### yamlsource
- [pyyaml; extra=='yamlsource'](packages/p/pyyaml.md)


## Publishers
- [piotrmaslanka](https://pypi.org/user/piotrmaslanka)


## Yanked Versions
- [2.11.20](https://pypi.org/project/satella/2.11.20)
- [2.11.32](https://pypi.org/project/satella/2.11.32)
- [2.11.33](https://pypi.org/project/satella/2.11.33)
- [2.12.4](https://pypi.org/project/satella/2.12.4)
 