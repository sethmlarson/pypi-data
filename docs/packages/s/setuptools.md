# [setuptools](https://pypi.org/project/setuptools)


## Extras

### certs
- [certifi (==2016.9.26); extra=='certs'](packages/c/certifi.md)

### ssl
- [wincertstore (==0.2); (sys_platform=='win32') and extra=='ssl'](packages/w/wincertstore.md)


## Publishers
- [dstufft](https://pypi.org/user/dstufft)
- [ianb](https://pypi.org/user/ianb)
- [Ivoz](https://pypi.org/user/Ivoz)
- [jaraco](https://pypi.org/user/jaraco)
- [jezdez](https://pypi.org/user/jezdez)
- [pje](https://pypi.org/user/pje)

